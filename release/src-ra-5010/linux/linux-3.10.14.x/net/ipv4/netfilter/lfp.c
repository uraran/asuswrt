...
static int lfp_ctrl(struct file *file, const char __user *buffer, size_t length, loff_t *data)
#else
...
#if LINUX_VERSION_CODE > KERNEL_VERSION(3,10,13)
static int lfp_read_proc(struct file *file, __user char *buffer, size_t count, loff_t *data)
#else
...
#if LINUX_VERSION_CODE > KERNEL_VERSION(3,10,13)
static int lfp_port_write_proc(struct file *file, const char __user *buffer, size_t length, loff_t *data)
#else
...
#if LINUX_VERSION_CODE > KERNEL_VERSION(3,10,13)
static int lfp_port_read_proc(struct file *file, __user char *buffer, size_t count, loff_t *data)
#else
...
#ifdef CONFIG_PROC_FS
#if LINUX_VERSION_CODE > KERNEL_VERSION(3,10,13)
static const struct file_operations lfpctrl_proc_ops = {
	.read		= lfp_read_proc,
	.write		= lfp_ctrl,
};

static const struct file_operations lfp_ports_ctrl_proc_ops = {
	.read		= lfp_port_read_proc,
...
#if LINUX_VERSION_CODE > KERNEL_VERSION(3,10,13)
	p = proc_create("lfpctrl", 0600, init_net.proc_net, &lfpctrl_proc_ops);
#else
...
#if LINUX_VERSION_CODE > KERNEL_VERSION(3,10,13)
	p = proc_create("lfp_ports_ctrl", 0600, init_net.proc_net, &lfp_ports_ctrl_proc_ops);
#else
